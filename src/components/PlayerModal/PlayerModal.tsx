...
const PlayerModal: React.FC<SelectedPlayerProps> = ({ onClose }) => {
  ...
  const selectedPlayer = useSelector(
    (state: RootReducerProp) => state.selected.selectedPlayer
  );
  ...
  const handleKillOrRevive = () => {
    let isDeadVariable =
      selectedPlayer.player.isDead === "TRUE" ? "FALSE" : "TRUE";
    setPlayerNameError(false);
    setPlayerNameHelperText("");
    if (selectedPlayer && selectedPlayer.id) {
      campaignsRef
  ...
  const toggleInputChange = (update: any) => {
    ...
    switch (update.type) {
      case "playerName":
        setPlayerName(update.payload);
        break;
      case "characterName":
        setCharacterName(update.payload);
        break;
      case "playerRace":
        setPlayerRace(update.payload);
        break;
      case "playerClass":
        setPlayerClass(update.payload);
        break;
      case "playerLevel":
        setPlayerLevel(update.payload);
        break;
    }
  };
  const handleClose = () => {
