...
const MonsterNew: FunctionComponent<MonsterNewProps> = () => {
  ...
  useMemo(() => {
    fetch(`${MONSTER_DATABASE_API}/api/monsters`)
      .then((res) => res.json())
      .then((json) =>
        json.results.map((apiMonster: ApiType) =>
          setMonsterChoices(
            (existing: { title: string; content: ApiType }[]) => [
              ...existing,
              { title: apiMonster.name, content: apiMonster },
            ]
          )
        )
      )
  ...
  const submitMonster = (monster: IMonster) => {
    if (selectedCampaignDatabasePath && selectedCampaign) {
      database
        .ref(selectedCampaignDatabasePath + "/monsters")
        .push(monster)
        .then((snapshot) => {
          let characterId = snapshot.key;
          if (characterId) {
            dispatch(setSelectedMonster({ id: characterId, monster: monster }));
            history.push(
  ...
  const createMonster = () => {
    ...
    if (monsterTemplate) {
      fetch(`${MONSTER_DATABASE_API}${monsterTemplate.url}`)
        .then((res) => res.json())
        .then((monsterJson) => {
          ...
          return {
            ...newMonster,
            alignment: monsterJson.alignment
              ? monsterJson.alignment
              : NEW_MONSTER.alignment,
            actions: monsterJson.actions ? monsterJson.actions : [],
            challengeRating: monsterJson.challenge_rating
            ...
            languages: monsterJson.languages
              ...
              : [],
            legendaryActions: monsterJson.legendary_actions
              ? monsterJson.legendary_actions
              : [],
            index: monsterJson.index ? monsterJson.index : newMonster.slug,
            senses: monsterJson.senses
              ? monsterJson.senses
              : NEW_MONSTER.senses,
            ...
            specialAbilities: monsterJson.special_abilities
              ? monsterJson.special_abilities
              : [],
          ...
          >
            {monsterChoices && !isFetchingMonsterList ? (
              <Autocomplete
                ...
                onChange={(
                  event: any,
                  monsterTemplate: { title: string; content: ApiType } | null
                ) => {
                  if (monsterTemplate) {
            ...
            <Tooltip
              title={
                translate.t(`Template monsters from`) +
