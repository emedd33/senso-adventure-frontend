...
import { Accordion, AccordionActions, AccordionDetails, AccordionSummary, Button, Typography, Divider } from "@material-ui/core";
...
const CampaignCharacters: FunctionComponent<CampaignCharactersProps> = () => {
    ...
    return (
        <Container >
            {characters && selectedCampaign ? characters.map(([, character]: [string, ICharacter,]) => (<Accordion style={character.isPublished === "TRUE" ? { backgroundColor: OLD_WHITE } : { backgroundColor: OLD_WHITE, opacity: 0.5 }} >
                <AccordionSummary
                ...
                >
                    ...
                    <Typography style={{ fontSize: "1rem", opacity: 0.7 }}>{character.isPlayer === "TRUE" ? "Player" : "NPC"}</Typography>
                </AccordionSummary>
                <AccordionDetails style={{ display: "grid", gridTemplateColumns: "3fr 1fr 5fr" }}>
